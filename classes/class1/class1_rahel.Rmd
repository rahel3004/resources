...
knitr::opts_knit$set(root.dir = "/Users/rahel/Desktop/Methods 2/resources")
...
Create some fictional data that behave like the one you expect
...
```{r}
  ...
  geom_histogram(binwidth = 1, fill = 'plum', color = 'purple')
...
They are a little bit different, because the sample size is not as big and the samples are picked randomly from this distribution
```{r}
mean(my_simulated_data) #5.25
sd(my_simulated_data)  #2.248
```
...
#create random data in range:
x_values <- seq(0,4,length.out= 1000)
#calculate values
y_values_line <- intercept + slope*x_values

# generate data with same residual sd (add normally distributed random errors with sd 3.9)
set.seed(123)
y_values_data <- y_values_line + rnorm(length(x_values),mean = 0, sd = residual_sd)
data <- data.frame(
  x= x_values,
  y_line = y_values_line,
  y_data =y_values_data)
#make plot
p <- data %>% ggplot(aes(x= x))+
  geom_point(aes(y= y_data), alpha = 0.6, color = 'blue')+
...
# get y_line
y_line <- intercept + slope*x_values

# add error:
set.seed(123)
y_data <- y_line + rnorm(x_values, mean = 0, sd = 10)
...
# plot
data2 %>% ggplot(aes( x=x))+
  geom_point( aes( y = y_data), color = 'purple')+
  geom_line(aes (y = y_line), color = 'black')+
...
#### (b) Exploring associations (more general relationship then correlation)
- association between global temperature and green house gas emission
-association between effect of new teaching strategy

#### (c) Extrapolation
- generalize from sample (e.g. cogsci students) to population (e.g. students) ?
...
```{r}
#load in the data
setwd('C:/Users/rahel/Desktop/Methods 2/resources/classes/data/Names/data')
names_data <- read.csv('allnames_clean.csv')
unique(names_data$sex)
# only girls names
female_names <- names_data %>% dplyr::filter(sex == 'F')
unique(female_names$sex) #check 
#make a column with last letters
female_names <- female_names %>% mutate(letter= str_sub(female_names$name, -1))
female_names <- female_names %>% mutate(letter = as.factor(letter))
colnames(female_names)

# get years
years <- grep('^X\\d{4}$',names(female_names),value = TRUE) #were we supposed to come up with this -> what exactly does it mean?
# summarise number of those per year for each letter
sums <- female_names %>% group_by(letter) %>% summarise_at(years,sum,na.rm = TRUE)
#turn into percentages
freq_to_perc <- function(x,na.rm = TRUE) (x/sum(x))*100
percentage_names_f <- mutate_at(sums,
                                years,
                                freq_to_perc)
# one row for each year with each letter
long_df <- pivot_longer(percentage_names_f,cols = years, names_to = 'Year', values_to = 'numbers')
long_df <- long_df %>% mutate (Year = as.factor(Year))
long_df <- long_df %>%  mutate(Year = as.numeric(Year))
# fix year column (switched to 1)
long_df$Year <- long_df$Year+1879
max(long_df$Year)

#lets plot finally :)
long_df %>% 
  ggplot(aes(x = Year, y= numbers, col = letter, group = letter))+
  geom_line()+
