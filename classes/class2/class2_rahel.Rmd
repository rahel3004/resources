...
A for loop runs a special amount of time. A while loop runs until a special condition is no longer true

### Ex. B: When would you use for-loops and when would you use while-loops? Explain in your own words in 1-2 sentences
I would use a for-loop when i now how often the loop should run and a while loop, when it only should run until a condition changes
...
while(some_number < 20){
  print('the number is under 20 so i still run')
  print(some_number)
...
clothing_advice <- function(temperature) {
  if (temperature > 25 ) {
    ...
    print('You should war a light jacket!')
  }else print('You should wear a warm coat!')
...
```{r}
# simulate 400 births and figure out how many are girls
n_girls <- rbinom(1,400,0.488) # prob of girls birth is 48.8%
print(n_girls)

# get sense of distribution-> simulate 1000 times
n_sims <- 1000
...
hist(n_girls)
#n_girls <- rbinom(n_sims, 400, 0.488) # also possible
...
n_sims <- 1000 # spezify number for repeated simulations
n_girls <- rep(NA, n_sims) # count variable 
for (s in 1:n_sims){
birth_type <- sample(c("fraternal twin","identical twin","single birth"),
size=400, replace=TRUE, prob=c(1/125, 1/300, 1 - 1/125 - 1/300))
girls <- rep(NA, 400)
...
n_girls[s] <- sum(girls) # summarize count of loop to count variable
}
hist(n_girls)
```
...
basketball <- function(){
  ...
  results <- c(shots,score)
 return(results)
}
# Example usage of the function, when done
basket_sim <- replicate(100,basketball())
hist(basket_sim)
```
...
basket_sim_loop <- data.frame(Shots = numeric() ,Score = numeric())
for (i in 1: 1000){
  result <- basketball()
  new_row <- data.frame(Shots= result[1], Score = result[2])
  basket_sim_loop <- rbind(basket_sim_loop,new_row)
}
mean(basket_sim_loop$Shots)
sd(basket_sim_loop$Shots)
hist(basket_sim_loop$Shots)
```
...
```{r}
basket_sim_loop <- basket_sim_loop %>% mutate(Ratio = Score/Shots*100)
  Scatter_basket_sim <- ggplot(basket_sim_loop, aes(x = basket_sim_loop$Shots,y = basket_sim_loop$Ratio))+
  geom_point(color = 'blue' )+
    labs(y = '% of throws beeing successful',
         x = 'overall shots taken')
Scatter_basket_sim
```
...
```{r}
three_outof_ten <- dbinom(3,10,0.4)
...
# loop it
n_sims= 10000
score_count <- rep(NA,n_sims)
for (s in 1:n_sims){
  score_count[s] <- sim_10()
}
hist(score_count)
counting <- sum(score_count==3)
prob_three_hits <- counting/n_sims
prob_three_hits
three_outof_ten
```
...
df <- data.frame(values = sim_x)
df %>% ggplot(aes(x = values))+
  geom_density()+
  stat_function(fun = dnorm, args = list(mean=mean(sim_x), sd = sd(sim_x)),
                color = 'plum', size = 1)
```
...
```{r}
# compute heights of men
n_sims= 1000
height_dif_sims <-rep(NA,n_sims)
for (i in 1:n_sims){
  avg_men<- rnorm(100,69.1,2.9)
  x <- mean(avg_men)
  avg_women <- rnorm(100,mean = 63.7, sd = 2.7)
  y <- mean(avg_women)
  height_dif_sims[i] <- (x-y)
}
hist(height_dif_sims)
mean(height_dif_sims)
sd(height_dif_sims)
```
...
df_sims <- data.frame(V1= rlnorm(127,meanlog= 0.10,sdlog= 0.17))-1
for (i in 2:1000){
  df_sims[,i] <-rlnorm(127,meanlog= 0.10,sdlog= 0.17)-1 #why do i have to do the shifting with -1?!
}
```
...
conf_int <- function(x){
  mean_value <-  mean(x) # compute mean
  n <-  length(x)
...
upper_ci <- c()
for (i in 1:1000){
  lower_ci[i] <- conf_int(df_sims[,i])[1] #again not sure about df sims why [,i]??
  upper_ci[i] <- conf_int(df_sims[,i])[2]
}
...
```{r}
temp <- c()
# loop trough each column in df_sim
for (i in 1:ncol(df_sims)){
  #run test and save result in temp[]
  temp[i] <- t.test(df_sims[,i],mu=0, alternative = "greater")$p.value
}
sum(temp>=0)
```
